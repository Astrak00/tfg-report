\begin{figure}[h]
    \centering
    \begin{tikzpicture}
    %% Add a title for the figure
    \begin{semilogyaxis}[
        title={Logarithmic energy consumption - Laptop},
        width=\plotwidthgraph,
        height=\plotheightgraph,
        xlabel={Number of Cores},
        ylabel={Energy (J)},
        ymode=log,
        xmode=linear,
        grid=both,
        minor tick num=1,
        grid style={gray!30,dashed},
        xtick={1,2,4,8,10,14},
        x tick label style={
          font=\footnotesize,
          rotate=45,
          anchor=north east
        },
        legend style={
          at={(0.98,0.8)},
          anchor=north east,
          font=\scriptsize,
          nodes={scale=0.8,transform shape},
          draw=none
        },
        legend columns=2,
        transpose legend,
        legend cell align=left,
          ]
          %% C++ %%
          \addplot[
        blue,
        only marks,
        mark=o,
        mark options={draw=blue,fill=white}
          ]
          table[row sep=\\] {
        x    y      \\
        1    58.79  \\
        2    59.72  \\
        4    57.32  \\
        8    55.94  \\
        10   55.00  \\
        14   48.39  \\
          };
          \addlegendentry{C++}
          % Updated power‐law fit: y = 60.0 * x^(-0.075)
          \addplot[
        blue,
        forget plot,
        domain=1:14,
        samples=200
          ] {60.0 * x^(-0.075)};
          
          %% Go %%
          \addplot[
        violet,
        only marks,
        mark=o,
        mark options={draw=violet,fill=white}
          ]
          table[row sep=\\] {
        x     y       \\
        1     429.23  \\
        2     370.01  \\
        4     354.64  \\
        8     348.14  \\
        10    336.88  \\
        14    300.66  \\
          };
          \addlegendentry{Go}
          % Updated power‐law fit: y = 445 * x^(-0.145)
          \addplot[
        violet,
        forget plot,
        domain=1:14,
        samples=200
          ] {445 * x^(-0.145)};
          
          %% PyPy 3.11.11 %%
          \addplot[
        orange,
        only marks,
        mark=o,
        mark options={draw=orange,fill=white}
          ]
          table[row sep=\\] {
        x    y      \\
        1    194.69 \\
        2    211.21 \\
        4    216.53 \\
        8    216.60 \\
        10   214.96 \\
        14   203.92 \\
          };
          \addlegendentry{PyPy 3.11.11}
          % Updated power‐law fit: y = 205 * x^(0.015)
          \addplot[
        orange,
        forget plot,
        domain=1:14,
        samples=200
          ] {205 * x^(0.015)};
          
          %% Python %%
        \addplot[
        green!60!black,
        only marks,
        mark=o,
        mark options={draw=green!60!black,fill=white}
          ]
        table[row sep=\\] {
        x      y        \\
        1      17546.05 \\
        2      16322.57 \\
        4      16193.99 \\
        8      15509.12 \\
        10     14942.09 \\
        14     13777.34 \\
        };
        \addlegendentry{Python}
        % Updated power‐law fit: y = 20000 * x^(-0.030)
        \addplot[
          green!60!black,
          forget plot,
          domain=1:14,
          samples=200
        ] {17563.62 * x^(-0.074)};

    \end{semilogyaxis}
\end{tikzpicture}
    \caption[Laptop - Logarithmic energy consumption]{Logarithm energy consumption of the Laptop benchmark across different programming languages. source: \autoref{tab:mbp-power-consumption}}
    \label{fig:log-mbp-energy}
\end{figure}

\begin{figure}
    \centering
    \begin{tikzpicture}
    \begin{axis}[
      title={Linear energy consumption without Python - Laptop},
      width=\plotwidthgraph,
      height=\plotheightgraph,
      xlabel={Number of Cores},
      ylabel={Energy (J)},
      ymode=linear,
      xmode=linear,
      grid=both,
      minor tick num=1,
      grid style={gray!30,dashed},
      xtick={1,2,4,8,10,14},
      x tick label style={
        font=\footnotesize,
        rotate=45,
        anchor=north east
      },
      legend style={
        at={(0.98,0.98)},
        anchor=north east,
        font=\scriptsize,
        nodes={scale=0.8,transform shape},
        draw=none
      },
      legend columns=2,
      transpose legend,
      legend cell align=left,
    ]
    %% C++ %%
    \addplot[
      blue,
      only marks,
      mark=o,
      mark options={draw=blue,fill=white}
    ]
    table[row sep=\\] {
      x    y      \\
      1    58.79  \\
      2    59.72  \\
      4    57.32  \\
      8    55.94  \\
      10   55.00  \\
      14   48.39  \\
    };
    \addlegendentry{C++}
    % power‐law fit: y = 64.0 * x^(-0.081)
    \addplot[
      blue,
      forget plot,
      domain=1:14,
      samples=200
    ] {64.0 * x^(-0.081)};

    %% Go %%
    \addplot[
      violet,
      only marks,
      mark=o,
      mark options={draw=violet,fill=white}
    ]
    table[row sep=\\] {
      x     y       \\
      1     429.23  \\
      2     370.01  \\
      4     354.64  \\
      8     348.14  \\
      10    336.88  \\
      14    300.66  \\
    };
    \addlegendentry{Go}
    % power‐law fit: y = 453 * x^(-0.140)
    \addplot[
      violet,
      forget plot,
      domain=1:14,
      samples=200
    ] {453 * x^(-0.140)};

    %% PyPy 3.11.11 %%
    \addplot[
      orange,
      only marks,
      mark=o,
      mark options={draw=orange,fill=white}
    ]
    table[row sep=\\] {
      x    y      \\
      1    194.69 \\
      2    211.21 \\
      4    216.53 \\
      8    216.60 \\
      10   214.96 \\
      14   203.92 \\
    };
    \addlegendentry{PyPy 3.11.11}
    % power‐law fit: y = 203 * x^(0.021)
    \addplot[
      orange,
      forget plot,
      domain=1:14,
      samples=200
    ] {203 * x^(0.021)};

  \end{axis}
\end{tikzpicture}
    \caption[Laptop - Linear energy consumption]{Linear energy consumption of the laptop benchmark across different programming languages without Python. source: \autoref{tab:mbp-power-consumption}}
    \label{fig:linear-mbp-energy}
\end{figure}

