...
Energy consumption in the software industry has been rising over the years to the point where it is now significant on a global scale. % `Insert reference to papers that states how much energy software produces, not in embedded but in laptops, renderer (not washingmachines)`
...
Once the code is created, the methodology for testing the different codes need to also be created. The platforms to be used for testing are:
\begin{itemize}
