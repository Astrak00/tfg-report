...
TODO: poner que es pkg y ram. y en mayus RAM
...
We can also observe from \autoref{fig:linear-desktop-energy-hyperthreading} and \autoref{fig:linear-desktop-execution-time-hyperthreading} that the difference between the 8-core and 16-core tests is less pronounced compared to the server. This is because the desktop processor has only 8 physical cores, while the remaining 8 are hyperthreaded cores, which are less powerful than the physical ones while the server has 16 physical cores.
...
\input{parts/results/mbp-wattage.tex}
\input{parts/results/mbp-speedup.tex}
