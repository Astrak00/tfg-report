...
Section \ref{sec:general_program_design} describes the general design of the programs, and how they are divided. Section \ref{sec:scene} describes the implementation of the scene objects and their interactions. In both \autoref{sec:sphere-object} and \autoref{sec:renderer}, we will see how the objects are designed and how the rendering is done, respectively. Then, \autoref{sec:output} describes how the output is generated and how it can be used to visualize the results of the ray-tracer and finally, the last section, \ref{sec:running_program}, describes how the program can be run and how to use the \texttt{Makefile} to run the program with different parameters.

\section{General Program Design}
\label{sec:general_program_design}
The `30,000 feet view' of the programs is a ray-tracer, that has multiple spheres, with different materials, indexes of refraction and sizes. Each of the pixels is calculated individually, and then, when all pixels have been processed, they are outputted to a \texttt{ppm} file.
...
\begin{itemize}
    ...
    \item \textbf{samplesPerPixel <int>} $\Rightarrow{}$ The number of rays per pixel used for anti-aliasing. The higher this number is, the slower the render, but the less noise that the output image has. See \autoref{fig:ray_traced_average} and \autoref{fig:ray_traced_max} for examples of the same image with different \texttt{samplesPerPixel} values.
    ...
    \item \textbf{defocusAngle <double>} $\Rightarrow{}$ This parameter represents the `aperture'\footnote{If it were to be a digital camera}. A higher number will mean more objects will be in focus, and a smaller number results in a shallower depth of field.
...
\subsubsection{C++}
When using C++, the intent was to use some of its modern features that would make development easier and adapted to new standards such as the use of smart pointers, \texttt{constexpr} and range-based loops. It was designed as an object-oriented program, with polymorphism though virtual functions (inside \texttt{material}, \texttt{hittable}).
...
This creates a job for every pixel row, running the function \texttt{self.process\_row} inside the camera object, with parameters \texttt{j} and \texttt{world}.

To retrieve the results, you can simply iterate the futures object as if it were a list of results:
\begin{lstlisting}[language=Python, caption={Python retrieving data from Process execution pool.}, label={lst:python_executor_retrieve}]
...
An interesting aspect that explains the difference between the loops in Python and the other languages is that compiled languages iterate over every pixel while Python iterates over every line. This happens because a copy of the Python environment must be created for some parallelization approaches, and the overhead of copying so much information slows the program significantly. We will see the impact of these changes in the evaluation section of the report.
...
The design of the output is a \gls{ppm} file, in which the first three lines define the content, aspect and maximum values of a file, once all pixels have been processed, the file is closed.
...
\autoref{fig:ray_traced_max} is an example image, of an image that this program would generate at maximum reasonable quality settings, 1920 width, 300 samples per pixel and 200 max depth.
...
And this \autoref{fig:ray_traced_average} is an example of the same picture, but with the settings used to test the programs on the personal laptop and server, which takes 168x more time than \autoref{fig:ray_traced_max}
...
To run the renderer, I have designed a \texttt{Makefile} that allows to run the program with different parameters, such as the number of cores to be used and where the user can specify the platform the benchmark is being run on. This is specially needed for macOS, as it needs \gls{sudo} privileges to perform the power measurements \footnote{This is a security risk, as it allows the program to have sudo privileges.}.
