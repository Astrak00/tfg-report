...
{Virtual Machine}
{The Python virtual machine is the same as the interpreter, which turns Python's code into machine ready instructions. Python's default VM implementation is \gls{CPython}, but others exist such as Jython (Java), IronPython (.NET) or PyPy}
...
\newglossaryentrywithacronym{CCX}{Core Complex}{
    A Core Complex (CCX) is a group of CPU cores that share a common cache and memory controller, allowing for efficient communication and data sharing between the cores within a single processor package. In AMD's Zen architecture, each CCX typically contains four cores and a shared L3 cache.}
...
\newglossaryentrywithacronym{SBC}
...
\newglossaryentrywithacronym{RAPL}
...
\newglossaryentrywithacronym{generic}
{generic}
