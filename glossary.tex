...
\newglossaryentry{runtime}{
    name = {runtime},
    description = {The runtime is a set of code that is executed while a program is running, which can include the program's memory management and other system resources it uses.}
}

\newglossaryentry{goroutine}{
    name = {goroutine},
    description = {A goroutine is a lightweight thread managed by the Go \gls{runtime}.}
}
...
\newglossaryentry{GIL}{
    ...
    description = {The global interpreter lock is a mutex that protects access to Python objects, preventing multiple threads from executing Python bytecodes at once}
}

\newglossaryentry{CPython}{
    name = {CPython},
    description = {Python's default interpreter, with a \gls{GIL}, until version 3.13 which an experimental free-threaded version was created and version 3.14 will continue its development (both known as 3.13t and 3.14t respectively)}
}

\newglossaryentry{VM}{
    name = {Virtual Machine},
    description = {The Python virtual machine is the same as the interpreter, which turns Python's code into machine ready instructions. Python's default VM implementation is \gls{CPython}, but others exist such as Jython (Java), IronPython (.NET) or PyPy}
}

\newglossaryentry{GC}{
    name = {Garbage Collector},
    description = {The garbage collector is an automatic memory management whose primary job is to reclaim memory that was allocated by the program but is no longer being used}
}
...
\newglossaryentry{AST}{
    ...
    description = {An abstract syntax tree is a data structure used in computer science to represent the structure of a program or code snippet. It is a tree representation of the abstract syntactic structure of text (often source code) written in a formal language.}
}
...
\newglossaryentry{numpy}{
    name = {numpy},
    description = {The most popular Python package for scientific computation with Python, using \gls{c-extension} for performance-critical code and providing support for large, multi-dimensional arrays and matrices, along with a collection of mathematical functions to operate on these arrays.}
}
...
\newglossaryentry{V8}{
    name = {V8},
    description = {Google's open-source, high-performance JavaScript and WebAssembly engine, written in C++, that powers the Google Chrome browser and the Node.js \gls{runtime} environment}
}
...
\newglossaryentry{NUMA}{
    ...
    description = {It's a computer memory design where the time it takes to access memory depends on which processor is accessing which memory location}
}

\newglossaryentry{CCX}{
    name= {Core Complex (CCX)},
    description = {
    A Core Complex (CCX) is a group of cores that share a common cache and memory controller, allowing for efficient communication and data sharing between the cores within a single processor package. In AMD's Zen architecture, each CCX typically contains four cores and a shared L3 cache.}
}
...
\newglossaryentry{oop}{
    ...
    description = {Programming paradigm that organizes code into reusable units called `objects', which contain both data and the functions that operate on that data}
}

\newglossaryentry{openMP}{
    name = {openMP},
    description = {OpenMP is an Application Program Interface (API) that may be used to explicitly direct multithreaded, shared memory parallelism in C/C++ programs. It is an extension for C/C++/Fortran}
}

\newglossaryentry{green-thread}{
    name = {Green threads},
    description = {Green threads are software threads managed by a \gls{runtime} environment or library, rather than directly by the operating system}
}
\newglossaryentry{wg}{
    name = {Wait Group},
    description = {A Go WaitGroup is a synchronization primitive that uses an internal counter, incremented by \texttt{Add} when goroutines start and decremented by \texttt{Done} when they finish, to block \texttt{Wait} until all registered goroutines have completed.}
}
...
\newglossaryentry{sudo}{
    name = {sudo},
    description = {sudo is a command-line utility in Unix-like operating systems that allows a permitted user to execute a command as the superuser or another user, as specified by the security policy. It stands for \textit{superuser do} and is commonly used to perform administrative tasks with elevated privileges.}
}
...
\newglossaryentry{fpga}{
    ...
    description = {A Field Programmable Gate Array is an integrated circuit that can be configured by the user after manufacturing to perform specific hardware tasks, allowing for customizable and reprogrammable logic functions.}
}

\newglossaryentry{OS}{
    name = {Operating System},
    description = {An operating system is software that manages computer hardware and software resources. It acts as an intermediary between users and the computer hardware, providing a user interface and managing system resources.}
}

\newglossaryentry{SBC}{
    name = {Single Board Computer},
    description = {A single-board computer is a complete computer built on a single circuit board, integrating all the necessary components such as the processor, memory, and input/output interfaces, making it compact and suitable for embedded applications.}
}

\newglossaryentry{ALU}{
    name = {Arithmetic Logic Unit},
    description = {A component of a computer's CPU that performs arithmetic and logical operations on binary data, serving as the fundamental building block for processing instructions in digital computers.}
}

\newglossaryentry{RAPL}{
    name = {Running Average Power Limit},
    description = {A feature in Intel processors that allows software to monitor and control the power consumption of the CPU and other components, helping to optimize energy efficiency and thermal management.}
}

\newacronym{ARM}{ARM}{Advanced \gls{RISC} Machine, originally Acorn \gls{RISC} Machine}

\newglossaryentry{generic}{
    name = {Generic},
    description = {Generics is a programming paradigm that enables the writing of code that is independent of any particular type. It allows algorithms to be written in a way that can operate on different data types without being rewritten for each type. This is often achieved through the use of templates or polymorphism.}
}

\newglossaryentry{mgp}{
    name = {M:P:G Model},
    description = {
        The M:P:G model is a concurrency model used in the Go programming language to manage the execution of goroutines. It consists of three main components: M (Machine), P (Processor), and G (Goroutine). \textbf{M} represents the operating system threads, \textbf{P} represents the logical processors that schedule and execute goroutines, and \textbf{G} represents the lightweight threads (goroutines) that are managed by the Go runtime. This model allows for efficient scheduling and execution of concurrent tasks in Go programs.
    }
}

\newglossaryentry{CUDA}{
    name = {CUDA},
    description = {CUDA (Compute Unified Device Architecture) is a parallel computing platform and programming model developed by NVIDIA that enables developers to harness the power of NVIDIA GPUs for general-purpose computing tasks, allowing for significant performance improvements in applications that require intensive computation.}
}

\newglossaryentry{Metal}{
    name = {Metal},
    description = {Metal is a low-level, low-overhead hardware-accelerated graphics and compute API created by Apple Inc. It provides near-direct access to the GPU on iOS, macOS, and tvOS devices, enabling developers to maximize the performance of their applications by efficiently utilizing the GPU for rendering graphics and performing parallel computations.}
}